...
def wcount(lines, topn=10):
    lst = list(map(str, lines.splitlines()))  # Turn the lines into a list of all the words which's "\n" and "\r" have been removed.
    wlist = []
    for i in lst:
        if i == lst[0]:
            wlist += i[14:len(i)-1].split()  # For the form of the first word in this txt is special, cut off 14 letters before the exact word.
        else:
            wlist += i[2:len(i)-1].split()  # Cut off the 'b' letter ahead and the "'" in the end.
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 1:  # If there is no input, print out the instruction.
        ...
        try:
            ...
            if sys.argv[2].isdigit():  # If input number after URL, choose the first number as topn.
                topn = int(sys.argv[2])
                wcount(docstr, topn)
